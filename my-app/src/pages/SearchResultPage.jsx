...
import { searchBlogByKeywordAPI, searchBlogWithFilter } from '../services/blogService.js'
...
const SearchResultPage = () => {
  ...
  const searchWithFilter = async () => {
    if (selectedCategories.length === 0 && selectedTags.length === 0) {
      ...
      try {
        const res = await searchBlogWithFilter(selectedCategories, selectedTags, page, size)
        setBlogs(res.result)
        const bloggerIds = res.result.map(blog => blog.author.userId)
        getBloggerByIds(bloggerIds)
      } catch (e) {
